...
/**
 * Celda contenedora que contiene el grafo de dependencia.
 * 
 * @author David Pastor Herranz
 */
public class ContenedorGrafoDependencia {
	...
	private ContenedorGrafoDependencia contenedoresHijos[];
	 ...
	 */
	public ContenedorGrafoDependencia(RegistroActivacion ra, JGraph graph,
		...
		if (this.nivel == 1) {
			...
			ContenedorGrafoDependencia.objetoNivel = null;
		...
		if (ContenedorGrafoDependencia.objetoNivel == null) {
		...
		this.contenedoresHijos = new ContenedorGrafoDependencia[(Conf.mostrarArbolColapsado ? ra
			...
			// nodos futuros, aún no visibles
		{
				...
				// (con históricos)
			{
				for (int i = 0; i < this.contenedoresHijos.length; i++) {
					this.contenedoresHijos[i] = new ContenedorGrafoDependencia(
				...
				// históricos)
			{
				...
				for (int i = 0; i < numHijos; i++) {
					if (!ra.getHijo(i).esHistorico()
						...
						this.contenedoresHijos[x] = new ContenedorGrafoDependencia(
								ra.getHijo(i), graph, this.nyp, this.nivel + 1);
						x++;
						this.tieneHijos = true;
					}
			...
			for (int i = 0; i < numHijos; i++) {
				this.contenedoresHijos[i] = new ContenedorGrafoDependencia(ra.getHijo(i),
