/**
	Genera el código java resultante de incorporar líneas de traza, basado en el Document manipulado por el Transformador.
	Clase de servicios.
...
{
	static boolean ver_codigo_terminal=false;	// true = se muestra código java generado por terminal
	...
	*/
	...
	/**
		...
		@param d Document que se escribirá en fichero
		@param fw FileWriter donde se irá volcando el contenido del fichero
		@param tab número de tabulaciones que se deben escribr antes de cada línea
		@param n2 nodo siguiente. En algunas ocasiones necesitamos conocer atributos del siguiente nodo (al declarar varias variables a la vez)
	*/
	private static void writeJavaFile(Node n, FileWriter fw, int tab, Node n2)
	{
		...
		if (n instanceof Element)
		{
			...
			else if (e.getNodeName().equals("package-decl"))
			{
				...
				// La comentamos porque es mejor que no escriba nada de paquetes, la clase SRec_ no compilará
			}
			...
			else if (e.getNodeName().equals("class")) // Nodo de la clase
			{
				...
				for (int i=0; i<el.length; i++)
					if (el[i].getNodeName().equals("implement"))
						if (!mas_implements)
						{
							s=s+"implements "+el[i].getAttribute("interface");
							mas_implements=true;
						}
						else
				...
				for (int i=0; i<el.length; i++)
					if (!(el[i].getNodeName().equals("superclass") || el[i].getNodeName().equals("implement")) )
					{
			...
			else if ( (e.getNodeName().equals("field")) || (e.getNodeName().equals("local-variable")) ) // Nodo atributo de la clase
			{
			...
			{	
				...
				if (el[0].hasAttribute("dimensions"))
					//for (int i=0; i<Integer.parseInt(el[0].getAttribute("dimensions")); i++)
					//	s=s+"[]";
					s=s+ServiciosString.cadenaDimensiones( Integer.parseInt(el[0].getAttribute("dimensions") ));
				...
				for (int i=0; i<el.length; i++)
					if (el[i].getNodeName().equals("formal-arguments"))
						s=s+stringParametros(el[i],ManipulacionElement.getChildElements(el[i]));

				s=s+" )";
				for (int i=0; i<el.length; i++)
					if (el[i].getNodeName().equals("throws") && mas_throws==false)
					{
						s=s+" throws "+el[i].getAttribute("exception");
						mas_throws=true;
					}
					else if (el[i].getNodeName().equals("throws") && mas_throws==true)
			...
			// ***************************************** NODO this-call o super-call

			else if (e.getNodeName().equals("this-call") || e.getNodeName().equals("super-call"))
			{
			...
			// ***************************************** NODO loop (bucles for, while y do)
			...
			else if (e.getNodeName().equals("loop"))
			{
				if (e.getAttribute("kind").equals("for"))
				{
					...
					if (init!=null)
					{
						...
						{
							if (initHijos[i].getNodeName().equals("local-variable"))
								s=s+stringFieldLocalVariable(initHijos[i],ManipulacionElement.getChildElements(initHijos[i]));
							else if (initHijos[i].getNodeName().equals("assignment-expr"))
								s=s+stringAssignment(initHijos[i],ManipulacionElement.getChildElements(initHijos[i]));
							else if (initHijos[i].getNodeName().equals("conditional-expr"))
								s=s+stringConditional(initHijos[i],ManipulacionElement.getChildElements(initHijos[i]));
							if (i<initHijos.length-1) // Si hay más cosas en la inicialización de for
								s=s+", ";
							else
								s=s+"; ";
						}
					...
					if (test!=null)
					{
						...
						for (int i=0; i<testHijos.length; i++)
						{
							...
							if (i<testHijos.length-1) // Si hay más cosas en la actualización de for
								s=s+", ";
							else
								s=s+"; ";
						}
					...
					if (update!=null)
					{
						updateHijos = ManipulacionElement.getChildElements(update);

						// Espacio de actualización del bucle for
						for (int i=0; i<updateHijos.length; i++)
						{
							if (updateHijos[i].getNodeName().equals("binary-expr"))
								s=s+stringExpresion(updateHijos[i],ManipulacionElement.getChildElements(updateHijos[i]));
							else if (updateHijos[i].getNodeName().equals("unary-expr"))
								s=s+stringExpresionU(updateHijos[i],ManipulacionElement.getChildElements(updateHijos[i]));
							else if (updateHijos[i].getNodeName().equals("send"))
								s=s+stringExpresion(updateHijos[i],ManipulacionElement.getChildElements(updateHijos[i]));
							else if (updateHijos[i].getNodeName().equals("field-access"))
								s=s+stringSendTarget(updateHijos[i],ManipulacionElement.getChildElements(updateHijos[i]))+"."+updateHijos[i].getAttribute("field");
							else if (updateHijos[i].getNodeName().equals("assignment-expr"))
							        s=s+stringAssignment(updateHijos[i],ManipulacionElement.getChildElements(updateHijos[i]));

							if (i<updateHijos.length-1) // Si hay más cosas en la inicialización de for
								s=s+", ";
							else
								s=s+")\r\n";
						}
					...
					esc(fw,conv(s));
					for (int i=0; i<el.length; i++)
						if (!(el[i].getNodeName().equals("init") || el[i].getNodeName().equals("test") || el[i].getNodeName().equals("update") || el[i].getNodeName().equals("block") ) )
							writeJavaFile(el[i],fw,tab+1,null); 
				...
				else if (e.getAttribute("kind").equals("while"))
				{
					...
					Element   testHijos[] = ManipulacionElement.getChildElements(test);
			...
			else if (e.getNodeName().equals("try"))
			{
				...
				Element elemCatch[]=ManipulacionElement.getChildElements(el[1]);
				s=s+(ManipulacionElement.getChildElements(elemCatch[0])[0]).getAttribute("name")+" "+elemCatch[0].getAttribute("name");
				s=s+")\r\n";
				esc(fw,conv(s));
			...
			else if (e.getNodeName().equals("if"))
			{
				...
				if (falseCase!=null)
					falseHijos = ManipulacionElement.getChildElements(falseCase);
			...
			else if (e.getNodeName().equals("switch"))
			{
				...
				if (el[0].getNodeName().equals("var-ref"))
					...
					s=s+stringExpresion(el[0],ManipulacionElement.getChildElements(el[0]));
				else if (el[0].getNodeName().equals("literal-null"))
					s=s+"null";
				else if (el[0].getNodeName().equals("this"))
					s=s+"this";
				else if (el[0].getNodeName().equals("send"))
					s=s+stringSend(el[0],ManipulacionElement.getChildElements(el[0]));
				else if (el[0].getNodeName().equals("field-access"))
					s=s+stringSendTarget(el[0],ManipulacionElement.getChildElements(el[0]))+"."+el[0].getAttribute("field");
			...
			// ***************************************** NODO switch-block (casos de switch, incluido default)
			...
			else if (e.getNodeName().equals("switch-block"))
			{
				...
				{
					s=s+"case ";
					Element valorTest[] = ManipulacionElement.getChildElements(el[0]);
					if (valorTest[0].getNodeName().equals("literal-number") || valorTest[0].getNodeName().equals("literal-char") || valorTest[0].getNodeName().equals("literal-boolean"))
						s=s+valorTest[0].getAttribute("value");
					else if (valorTest[0].getNodeName().equals("literal-string"))
						s=s+stringValorString(valorTest[0]);
					else if (valorTest[0].getNodeName().equals("literal-null"))
						s=s+"null";
					else if (valorTest[0].getNodeName().equals("binary-expr"))
						s=s+stringExpresion(valorTest[0],ManipulacionElement.getChildElements(valorTest[0]));
					else if (valorTest[0].getNodeName().equals("var-ref")) // Constantes - "final int"
						s=s+valorTest[0].getAttribute("name");
					else if (valorTest[0].getNodeName().equals("paren"))
						s=s+stringExpresionParen(valorTest[0],ManipulacionElement.getChildElements(valorTest[0]));
					else if (valorTest[0].getNodeName().equals("this"))
						s=s+"this";
					else if (valorTest[0].getNodeName().equals("send"))
						s=s+stringSend(valorTest[0],ManipulacionElement.getChildElements(valorTest[0]));
					else if (valorTest[0].getNodeName().equals("field-access"))
						s=s+stringSendTarget(valorTest[0],ManipulacionElement.getChildElements(valorTest[0]))+"."+valorTest[0].getAttribute("field");
				}
				else
					s=s+"default";
				s=s+":\r\n";
			...
			else if (e.getNodeName().equals("return"))
			{
				...
				if (el.length>0)
					if (el[0].getNodeName().equals("paren"))
						s=s+stringExpresionParen(el[0],ManipulacionElement.getChildElements(el[0]));
					else if (el[0].getNodeName().equals("cast-expr"))
						s=s+stringExpresionCasting(el[0],ManipulacionElement.getChildElements(el[0]));
					else if (el[0].getNodeName().equals("var-ref"))
						s=s+el[0].getAttribute("name");
					else if (el[0].getNodeName().equals("array-ref"))
						s=s+stringArrayRef(el[0],ManipulacionElement.getChildElements(el[0]));
					else if (el[0].getNodeName().equals("new-array"))
						s=s+stringNewArray(el[0],ManipulacionElement.getChildElements(el[0]));
					else if (el[0].getNodeName().equals("new"))
						s=s+stringNew(el[0],ManipulacionElement.getChildElements(el[0]));
					else if (el[0].getNodeName().equals("literal-number") || el[0].getNodeName().equals("literal-char"))
						s=s+el[0].getAttribute("value");
					else if (el[0].getNodeName().equals("literal-string"))
						s=s+stringValorString(el[0]);
					else if (el[0].getNodeName().equals("literal-boolean"))
						...
						s=s+stringInstanceOf(el[0],ManipulacionElement.getChildElements(el[0]));
					else if (el[0].getNodeName().equals("unary-expr"))
						s=s+stringExpresionU(el[0],ManipulacionElement.getChildElements(el[0]));
					else if (el[0].getNodeName().equals("binary-expr"))	
						s=s+stringExpresion(el[0],ManipulacionElement.getChildElements(el[0]));
					else if (el[0].getNodeName().equals("conditional-expr"))	
						s=s+stringConditional(el[0],ManipulacionElement.getChildElements(el[0]));
					else if (el[0].getNodeName().equals("send"))
						s=s+stringSend(el[0],ManipulacionElement.getChildElements(el[0]));
			...
			{
				s=s+"// "+e.getNodeName()+"(sentencia no reconocida)\r\n";
				esc(fw,conv(s));
			}
		}
	}
	...
	/**
		...
		@param e Element del que se va a sacar la representación
		@param el Lista de hijos de e
	...
	*/
	private static String stringFieldLocalVariable (Element e, Element el[]) 
	{
		...
		for (int i=0; i<el.length; i++)
			if (el[i].getNodeName().equals("type") && !(el[i].hasAttribute("dimensions"))) {
				if (e.hasAttribute("continued"))
					s=s+e.getAttribute("name");
				else
					s=s+el[i].getAttribute("name")+" "+e.getAttribute("name");
			}
			else if (el[i].getNodeName().equals("type") && el[i].hasAttribute("dimensions"))
			{
				s=s+el[i].getAttribute("name")+" "+e.getAttribute("name");
				//for (int j=0; j<Integer.parseInt(el[i].getAttribute("dimensions")); j++)
				//		s=s+"[]";
				s=s+ServiciosString.cadenaDimensiones( Integer.parseInt(el[i].getAttribute("dimensions") ));
			}

	
		for (int i=0; i<el.length; i++)		
			if (el[i].getNodeName().equals("array-initializer"))
				s=s+" = "+stringArrayInitializer(el[i],ManipulacionElement.getChildElements(el[i]));
			else if (el[i].getNodeName().equals("new-array"))
				s=s+" = "+stringNewArray(el[i],ManipulacionElement.getChildElements(el[i]));
			else if (el[i].getNodeName().equals("new"))
				s=s+" = "+stringNew(el[i],ManipulacionElement.getChildElements(el[i]));
			else if (el[i].getNodeName().equals("var-ref"))
				s=s+" = "+el[i].getAttribute("name");
			else if (el[i].getNodeName().equals("array-ref"))
				s=s+" = "+stringArrayRef(el[i],ManipulacionElement.getChildElements(el[i]));
			else if (el[i].getNodeName().contains("literal-") && !(el[i].getNodeName().contains("string")) && !(el[i].getNodeName().contains("null")))
				...
				s=s+" = "+stringSendTarget(el[i],ManipulacionElement.getChildElements(el[i]))+"."+el[i].getAttribute("field");
			else if (el[i].getNodeName().equals("unary-expr"))
				s=s+" = "+stringExpresionU(el[i],ManipulacionElement.getChildElements(el[i]));
			else if (el[i].getNodeName().equals("binary-expr"))
				s=s+" = "+stringExpresion(el[i],ManipulacionElement.getChildElements(el[i]));
			else if (el[i].getNodeName().equals("conditional-expr"))
				s=s+" = "+stringConditional(el[i],ManipulacionElement.getChildElements(el[i]));
			else if (el[i].getNodeName().equals("send"))
				s=s+" = "+stringSend(el[i],ManipulacionElement.getChildElements(el[i]));
			else if (el[i].getNodeName().equals("paren"))
				s=s+" = "+stringExpresionParen(el[i],ManipulacionElement.getChildElements(el[i]));
			else if (el[i].getNodeName().equals("cast-expr"))
				s=s+" = "+stringExpresionCasting(el[i],ManipulacionElement.getChildElements(el[i]));
	...
	*/
	private static String stringAssert(Element e, Element el[])
	{
		...
		s=s+"assert ";
		s=s+stringCondition(el[0],ManipulacionElement.getChildElements(el[0]));
		s=s+" : ";

		s=s+stringAssertErrorMessage(el[1],ManipulacionElement.getChildElements(el[1]));
	...
	*/
	private static String stringAssertErrorMessage(Element e, Element el[])
	{
		...
		if (el[0].getNodeName().equals("binary-expr"))
			...
			s=s+stringSendTarget(el[0],ManipulacionElement.getChildElements(el[0]))+"."+el[0].getAttribute("field");
		else if (el[0].getNodeName().contains("literal-") && !(el[0].getNodeName().contains("string")) && !(el[0].getNodeName().contains("null")))
			s=s+el[0].getAttribute("value");
			...
			s=s+stringArrayInitializer(el[0],ManipulacionElement.getChildElements(el[0]));
		else if (el[0].getNodeName().equals("new-array"))
			s=s+stringNewArray(el[0],ManipulacionElement.getChildElements(el[0]));
		else if (el[0].getNodeName().equals("new"))
			s=s+stringNew(el[0],ManipulacionElement.getChildElements(el[0]));
	...
	private static String stringCondition(Element e, Element el[])
	{
		...
		for (int i=0; i<el.length; i++)
		{
			if (el[i].getNodeName().equals("binary-expr"))
				...
				s=s+stringInstanceOf(el[i],ManipulacionElement.getChildElements(el[i]));
			else if (el[i].getNodeName().equals("conditional-expr"))
				s=s+stringConditional(el[i],ManipulacionElement.getChildElements(el[i]));
			else if (el[i].getNodeName().equals("send"))
				s=s+stringSend(el[i],ManipulacionElement.getChildElements(el[i]));
	...
	*/
	private static String stringDimExpr(Element e, Element el[])
	{
		...
		if (el.length>0)
			if (el[0].getNodeName().contains("literal-") && !el[0].getNodeName().contains("string") && !el[0].getNodeName().contains("null"))
				s=s+el[0].getAttribute("value");
			else if (el[0].getNodeName().contains("literal-string"))
				s=s+stringValorString(el[0]);
			else if (el[0].getNodeName().contains("literal-null"))
	...
	*/
 	private static String stringAssignment(Element e, Element el[]) 
	{
		...
		for (int i=0; i<el.length; i++)
		{
			...
			{
				Element hijosLValue[]=ManipulacionElement.getChildElements(el[i]);
				if (hijosLValue[0].getNodeName().equals("var-ref"))
					s=s+hijosLValue[0].getAttribute("name")+e.getAttribute("op");
				else if (hijosLValue[0].getNodeName().equals("array-ref"))
					s=s+stringArrayRef(hijosLValue[0],ManipulacionElement.getChildElements(hijosLValue[0]))+e.getAttribute("op");
				else if (hijosLValue[0].getNodeName().equals("field-access"))
					s=s+stringSendTarget(hijosLValue[0],ManipulacionElement.getChildElements(hijosLValue[0]))+"."+hijosLValue[0].getAttribute("field")+e.getAttribute("op");
			}
			if (el[i].getNodeName().equals("literal-number"))
				...
				s=s+stringAssignment(el[i],ManipulacionElement.getChildElements(el[i]));
		}
		return conv(s);
	}
	...
	private static String stringConditional(Element e, Element el[]) 
	{
		...
		for (int i=0; i<el.length; i++)
		{
			if (el[i].getNodeName().equals("literal-number"))
				...
				s=s+stringNew(el[i],ManipulacionElement.getChildElements(el[i]));
			if (i==0 && el.length>1) // Si hemos reconocido la condición y hay más nodos...
				s=s+" ? ";
			if (i==1 && el.length>2) // Si hemos reconocido el caso cierto y hay más nodos
				s=s+" : ";
		}
		return conv(s);
	}
	...
	private static String stringArrayRef(Element e, Element el[])
	{
		...
		for (int i=0; i<el.length; i++)
		{
			...
			if (el[i].getNodeName().equals("base"))
				...
				s=s+stringArrayBase(el[i],ManipulacionElement.getChildElements(el[i]));
			else	if (el[i].getNodeName().equals("offset"))
			{
				s=s+"[";		
				if (hijos[0].getNodeName().equals("paren"))
					s=s+stringExpresionParen(hijos[0],ManipulacionElement.getChildElements(hijos[0]));
				else if (hijos[0].getNodeName().equals("var-ref"))
					s=s+hijos[0].getAttribute("name");
				else if (hijos[0].getNodeName().equals("literal-number") || hijos[0].getNodeName().equals("literal-char"))
					...
					s=s+stringExpresionU(hijos[0],ManipulacionElement.getChildElements(hijos[0]));
				else if (hijos[0].getNodeName().equals("binary-expr"))
					s=s+stringExpresion(hijos[0],ManipulacionElement.getChildElements(hijos[0]));
				else if (hijos[0].getNodeName().equals("conditional-expr"))
					s=s+stringConditional(hijos[0],ManipulacionElement.getChildElements(hijos[0]));
				else if (hijos[0].getNodeName().equals("cast-expr"))
					s=s+stringExpresionCasting(hijos[0],ManipulacionElement.getChildElements(hijos[0]));
				else if (hijos[0].getNodeName().equals("field-access"))
					s=s+stringSendTarget(hijos[0],ManipulacionElement.getChildElements(hijos[0]))+"."+hijos[0].getAttribute("field");
				else if (hijos[0].getNodeName().equals("array-ref"))		//nuevo
					s=s+stringArrayRef(hijos[0],ManipulacionElement.getChildElements(hijos[0]));
				else if (hijos[0].getNodeName().equals("send"))
					s=s+stringSend(hijos[0],ManipulacionElement.getChildElements(hijos[0]));
				s=s+"]";		
			}
		}		
		return conv(s);
	...
	*/
	private static String stringArrayInitializer(Element e, Element el[])
	{
		...
		for (int i=0; i<el.length; i++)
		{
			if (el[i].getNodeName().contains("literal-") && !el[i].getNodeName().contains("string") && !el[i].getNodeName().contains("null"))
				s=s+" "+el[i].getAttribute("value");
			else if (el[i].getNodeName().equals("literal-string"))
				s=s+stringValorString(el[i]);
			else if (el[i].getNodeName().equals("literal-null"))
			...
			if (i<(el.length-1)) 	// Si no es ultimo valor...
				s=s+",";
			else				// Si sí que lo es...
				s=s+" }";				
		}
	...
	private static String stringNewArray(Element e, Element el[])
	{
		...
		for (int i=0; i<el.length; i++)
			if (el[i].getNodeName().equals("dim-expr"))
				s=s+"["+stringDimExpr(el[i],ManipulacionElement.getChildElements(el[i]))+"]";
	...
	private static String stringSend(Element e, Element el[])
	{
		...
		for (int i=0; i<el.length; i++)
			if (el[i].getNodeName().equals("target"))
				s=s+stringSendTarget(el[i],ManipulacionElement.getChildElements(el[i]))+".";
		s=s+e.getAttribute("message")+"(";
		for (int i=0; i<el.length; i++)
			if (el[i].getNodeName().equals("arguments"))
				s=s+stringSendArguments(el[i],ManipulacionElement.getChildElements(el[i]));
		s=s+")";
	...
	*/
	private static String stringSendTarget(Element e, Element el[])
	{
		...
		if (el[0].getNodeName().equals("paren"))
			...
			s=s+stringArrayRef(el[0],ManipulacionElement.getChildElements(el[0]))+e.getAttribute("op");
		else if (el[0].getNodeName().equals("this"))
			s=s+"this";
		...
		if (el.length>0)
			for (int i=0; i<el.length; i++)
			{
				if (el[i].getNodeName().equals("paren"))
					...
					s=s+stringSend(el[i],ManipulacionElement.getChildElements(el[i]));
				else if (el[i].getNodeName().equals("literal-number") || el[i].getNodeName().equals("literal-char"))
					s=s+el[i].getAttribute("value");
				else if (el[i].getNodeName().equals("literal-boolean"))
					s=s+el[i].getAttribute("value");
				else if (el[i].getNodeName().equals("literal-null"))
	...
	*/
	private static String stringExpresion(Element e, Element el[])
	{
		...
		// Adaptación para operadores modificados en JavaParser con el fin de que no se produzcan errores
		// ( '<' y '&' están "prohibidos" )
		...
		if (el[1].getNodeName().equals("paren"))
			s=s+stringExpresionParen(el[0],ManipulacionElement.getChildElements(el[1]));
		else if (el[1].getNodeName().equals("var-ref"))
			s=s+el[1].getAttribute("name");
		else if (el[1].getNodeName().equals("literal-number") || el[1].getNodeName().equals("literal-char"))
			...
			s=s+stringExpresionU(el[1],ManipulacionElement.getChildElements(el[1]));
		else if (el[1].getNodeName().equals("cast-expr"))
			s=s+stringExpresionCasting(el[1],ManipulacionElement.getChildElements(el[1]));
		else if (el[1].getNodeName().equals("field-access"))
			s=s+stringSendTarget(el[1],ManipulacionElement.getChildElements(el[1]))+"."+el[1].getAttribute("field");
		else if (el[1].getNodeName().equals("array-ref"))
			s=s+stringArrayRef(el[1],ManipulacionElement.getChildElements(el[1]));
		else if (el[1].getNodeName().equals("conditional-expr"))
			s=s+stringConditional(el[1],ManipulacionElement.getChildElements(el[1]));
		else if (el[1].getNodeName().equals("binary-expr"))
			s=s+stringExpresion(el[1],ManipulacionElement.getChildElements(el[1]));
		else if (el[1].getNodeName().equals("send"))
			s=s+stringSend(el[1],ManipulacionElement.getChildElements(el[1]));
		else if (el[1].getNodeName().equals("new"))
			s=s+stringNew(el[1],ManipulacionElement.getChildElements(el[1]));
		else if (el[1].getNodeName().equals("new-array"))
			s=s+stringNewArray(el[1],ManipulacionElement.getChildElements(el[1]));
		//else
		//	s="<stringExpresion1>";
	...
	/**
		Representación String de un nodo que representa expresiones encerradas en paréntesis
	...
	*/
	...
	/**
		Representación String de un nodo que representa expresiones unarias (  ++x, --y )
	...
	*/
	private static String stringExpresionU(Element e, Element el[])
	{
		...
		if (e.getAttribute("post").equals("true"))		// x++, y--
			s=s+e.getAttribute("op");
	...
	*/
	private static String stringParametros(Element e, Element argumentos[])
	{
		...
		{
			Element arg_tipo[]=ManipulacionElement.getChildElements(argumentos[i]);
			s=s+arg_tipo[0].getAttribute("name")+" "+argumentos[i].getAttribute("name");
			if (arg_tipo[0].hasAttribute("dimensions"))
				//for (int j=0; j<Integer.parseInt(arg_tipo[0].getAttribute("dimensions")); j++)
				//	s=s+"[]";
				s=s+ServiciosString.cadenaDimensiones( Integer.parseInt(arg_tipo[0].getAttribute("dimensions") ));
	...
	/**
		Método que realiza conversiones para lograr restaurar el estado original de cadenas de caracteres, nombres de variables, etc.
		
		@param s cadena con caracteres convertidos a secuencias
	...
	*/
	...
	/**
		Reinicializa una cadena para poder esciribr en ella una sentencia, una vez ha dejado la tabulación correcta
		
		@param tab número de espacios que se dejarán de tabulación
	...
	*/
	...
	public static void crearClase(String path)
	{
		...
		//System.out.println("crearClase ["+path+"]");
		
		String nombreClase=path.substring(0,path.lastIndexOf(".java"));
		nombreClase=nombreClase.substring(nombreClase.lastIndexOf(File.separator)+1);
	...
	/**
		...
		@param fw descriptor de fichero en el que se escribirá
		@param s cadena que se escribirá en el fichero
	*/
