...
//DO NOT REMOVE OR MODIFY THIS LINE!
	...
	public static final String VERSION = "JGraph (v5.11.0.1)";
	...
	public Graphics getOffgraphics() {
		...
		if ((offscreen == null || offgraphics == null || offscreenBounds == null)
			...
			if (!volatileOffscreen) {
				...
				graphLayoutCache.addOffscreenDirty((Rectangle2D)getBounds().clone());
			}
		}
		Rectangle2D offscreenDirty = graphLayoutCache.getOffscreenDirty();
		if (offscreenDirty != null) {
			...
			graphLayoutCache.clearOffscreenDirty();
		}
		return offgraphics;
	}
	...
	public Rectangle2D getClipRectangle(GraphLayoutCacheChange change) {
		Rectangle2D removed = getCellBounds(change.getRemoved());
		Rectangle2D inserted = getCellBounds(change.getInserted());
		Rectangle2D changed = getCellBounds(change.getChanged());
		Rectangle2D context = getCellBounds(change.getContext());

		Rectangle2D clip = removed;
