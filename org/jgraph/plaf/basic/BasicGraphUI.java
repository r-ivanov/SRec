...
//		}
	...
	public class GraphModelHandler implements GraphModelListener, Serializable {
		...
		public void graphChanged(GraphModelEvent e) {
			...
			Rectangle2D oldDirtyRegion = graph.getClipRectangle(e.getChange());
			graphLayoutCache.addOffscreenDirty(oldDirtyRegion);
			if (graphLayoutCache != null) {
			...
			graphLayoutCache.addOffscreenDirty(newDirtyRegion);
		...
		public void valueChanged(GraphSelectionEvent event) {
			...
			if (cells != null && cells.length <= MAXCLIPCELLS) {
				...
				if (r != null) {
					...
					graph.getGraphLayoutCache().addOffscreenDirty(r);
					graph.repaint(dirtyRegion);
				}
			} else {
				Rectangle dirtyRegion = (Rectangle)graph.getBounds().clone();
				graph.getGraphLayoutCache().addOffscreenDirty(dirtyRegion);
				graph.repaint();
			}
		}
	}
