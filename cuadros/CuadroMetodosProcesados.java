...
public class CuadroMetodosProcesados extends Thread implements ActionListener, KeyListener, MouseListener
{
	static final long serialVersionUID=02;
	private final boolean DEPURAR=false;

	final int anchoCuadro=720;
	final int altoCuadro=560;		// 130 aquí actúa como medida base inicial
	...
	*/
	public CuadroMetodosProcesados(ClaseAlgoritmo clase, Preprocesador p)
	{	
	...
	*/	
	public void run()
	{
		...
		if (this.numero>0)
		{
			...
			String textoBoton=Texto.get("CMP_BOTONPARAM", Conf.idioma);
			String toolTipBoton=Texto.get("CMP_TTBOTONPARAM",Conf.idioma);
			...
			for (int i=0; i<this.numero; i++)
			{
				...
				botonesOpcionesMetodo[i].setEnabled(this.metodos.get(i).getMarcadoVisualizar());
				botonesOpcionesMetodo[i].setToolTipText(toolTipBoton);
				panelDerecha.add(botonesOpcionesMetodo[i]);
				
				panelDerecha.setPreferredSize(new Dimension(95,28));
	...
	public void actualizarMetodo(MetodoAlgoritmo m,int numMetodo)
	{
		// Llamado desde el cuadro de parámetros nuevo, tras pulsar aceptar en ese cuadro
		
		this.metodos.remove(numMetodo);
		this.metodos.add(numMetodo,m);
			
		this.clase.addMetodo(m);
	...
	*/	
	public void recogerMetodoSeleccionado()
	{
		...
		// A través de este cuadro recogemos los parámetros también, por lo que el siguiente paso será llamar al preprocesador
		...
		for (int i=0; i<this.numero; i++)
		{
			...
			//System.out.println(i+": "+this.metodos.get(i).getNombre()+" = "+botonesRadio[i].isSelected()+" "+botonesCheck[i].isSelected());
		...
		for (int i=0; i<this.numero; i++)
		{
			if ( botonesRadio[i].isSelected() )
			{
				String []valoresParam=this.metodos.get(i).getParamValores();
				for (int j=0; j<valoresParam.length; j++)
				{
					if (valoresParam[j]==null || valoresParam[j].length()==0)
					{
						errorProducido=true;
						new CuadroError(vv , Texto.get("ERROR_PARAM",Conf.idioma) , Texto.get("CPARAM_NOVALOR",Conf.idioma)+" "+(j+1)+" ("+Texto.get("CMP_METPRINC",Conf.idioma)+" '"+this.metodos.get(i).getNombre()+"')");
					}
				}
			}
	...
	*/				
	...
	{
		//System.out.println("CuadrosMetodosProcesados.mouseReleased ("+")");
		...
		else if (e.getSource().getClass().getName().contains("JRadioButton"))
		{
			for (int i=0; i<this.numero; i++)
			{
				if (botonesRadio[i].isSelected())
				{
					...
					botonesOpcionesMetodo[i].setEnabled(true);
				}
			}
		}
		...
		else if (e.getSource().getClass().getName().contains("Boton"))
		{
			...
			for (int i=0; i<this.numero; i++)
			{
				...
				{
					//System.out.println("numMetodo="+i);
					new CuadroParam(this.metodos.get(i),this.clase,this,this.botonesRadio[i].isSelected(),i);
				}
			}
		}
