/**
	Gestiona toda la manipulación del árbol DOM, una vez se ha cargado el fichero XML del código java original.
	Clase de servicios.
...
public class Transformador {
	...
	static boolean depurar=false;				// DEPURAR
	
	/**
		Añade nodos al Document, en concreto, los de las líneas de traza
		
		@param d Document al que se añadirán nodos
		@param es nombre del método que se va a signar a la línea
		@param nombreArray nmbre del array que se pasa como aprámetro al método es
		@return Element añadido
	*/
	/*public static Element elementoAnadirEntradaSalida(Document d, String es, String nombreArray, 
														String nombreMetodo) 
	{
	...
	/**
		...
		@return Element añadido
	*/
	public static Element elementoAnadirEntrada(Document d, String es, String nombreArray, 
														String nombreMetodo) 
	{
	...
	/**
		...
		@return Element añadido
	*/
	public static Element elementoAnadirEntradaSalida(Document d, String es, String nombreArray, String nombreEstr, 
															int posicEstructuraEnParam, String nombreMetodo) 
	{
		...
		// 12. Creamos nodo var-ref y añadimos como hijo a elem13 (array de parámetros o valor de retorno)
		Element elem14 = d.createElement("var-ref");
		elem14.setAttribute("name",nombreArray);
		elem13.appendChild(elem14);
		...
		// 15. Creamos nodo var-ref y añadimos como hijo a elem13 (array de indices)
	...
	*/
	public static Element elementoAnadirEntradaSalida(Document d, String es, String nombreArray, String nombreEstr, 
															int posicEstructuraEnParam, String nombreMetodo, boolean devuelveValor) 
	{
	...
	/**
		...
		@return Element añadido
	*/
	public static Element elementoAnadirSalida(Document d, String es, String nombreArray, 
	...
	/**
		Ordena los nodos de las líneas de traza
		
		@param padre Node padre de los nodos de traza que se van a ordenar
		@param ret nodo correspondietne a la sentencia return que se encuentra en ese mismo bloque de instrucciones
		@param elementoAnadirEntrada elemento de entrada que se va a ordenar
		@param elementoAnadirSalida elemento de salida que se va a ordenar
	*/
	public static void colocarEntradaSalida(Node padre, Node ret, Element elementoAnadirEntrada, Element elementoAnadirSalida )
	{
		Node nodo_aux=null;
		// Recolocar elementoAnadirSalida:
		// Borramos nodo de return (sólo puede haber uno) y lo añadimos al final
		// de la lista de hijos de su padre

		if (elementoAnadirSalida!=null) // Si no queremos pasar nada para la salida...
		{
			padre.appendChild(elementoAnadirSalida);
			padre.removeChild(ret);
			padre.appendChild(ret);
		}
								
		// Recolocar elementoAnadirEntrada:
		// Recorremos hijos del padre, dejando pasar los nodos "local-variable" (y "#text").
		// Cuando pasen todos, incluimos elementoAnadirEntrada y después todas
		// las sentencias restantes del bloque

		if ( elementoAnadirEntrada!=null ) // Si no queremos pasar nada para la entrada...
		{
			NodeList hermanos=padre.getChildNodes();
	
			int j=0;
			while((hermanos.item(j).getNodeName().equals("local-variable") || 
				hermanos.item(j).getNodeName().equals("#text")) && 
				j<hermanos.getLength())
				j++;
	
			//if (depurar) System.out.println("Nodo que rompe bucle while: ("+j+") \""+hermanos.item(j).getNodeName());
	
			padre.appendChild(elementoAnadirEntrada);
	
			while(hermanos.item(j)!=elementoAnadirEntrada)
			{
				nodo_aux=hermanos.item(j);
				padre.removeChild(nodo_aux);
				padre.appendChild(nodo_aux);
			}
		}
	}
	...
	/**
		...
		@param d Document al que se añadirán nodos
		@param clase nombre de la clase a la que pertenece el array
		@param nombre nmbre del array que queremos declarar
		@param num_param número de valores que tendrá el array
		@return Element añadido
	*/
	public static Element elementoDeclaracion(Document d, String clase, String nombre ,int num_param)
	{
	...
	/**
		...
		@param d Document al que se añadirán nodos
		@param indice posición del array a la que vamos a asignar un valor
		@param nombre nombre del valor que vamos a asignar a una posición del array
		@param nombreArray nombre del array al que vamos a asignar un valor
		@return Element añadido
	*/
	public static Element elementosAsigParametros(Document d,int indice,String nombre, String nombreArray)
	{
	...
	/**
		...
		@param d Document al que se añadirán nodos
		@param indice posición del array a la que vamos a asignar un valor
		@param e Element que se incluye como valor que se asigna
		@param nombreArray nombre del array al que vamos a asignar un valor
		@return Element añadido
	*/
	public static Element elementosAsigParametros(Document d,int indice,Element e,String nombreArray)
	{
		// 01. Creamos nodo assignment-expr y añadimos al árbol
		Element elem01 = d.createElement("assignment-expr");
		elem01.setAttribute("op","=");
		...
		// 08. Insertamos nodo que devuelve valor de retorno en la asignación
		elem01.appendChild(e);
	...
	*/
	public static int insertarLineasNecesarias(Document d, ClaseAlgoritmo claseAlgoritmo)
	{
		...
		Element[] inspeccion_clase=ManipulacionElement.nodeListToElementArray(d.getElementsByTagName("class"));
		...
		// Necesitamos que la clase sea pública, no se lo exigimos a la clase de origen, pero sí a la que estamos creando
		Element elClase[] = ManipulacionElement.nodeListToElementArray(d.getElementsByTagName("class"));
		elClase[0].setAttribute("visibility","public");

		
			
		// - Buscamos todos los nodos de métodos
		// - Por cada metodo, si es void, le tratamos de forma especial (metemos lineas distintas)
		// - En caso contrario, por cada return, si está dentro de block actuamos normal y si no, lo metemos en block

		Element metodos[] = ManipulacionElement.nodeListToElementArray(d.getElementsByTagName("method"));
		...
		for (int i=0; i<metodos.length; i++)
			for (int j=0; j<claseAlgoritmo.getNumMetodos(); j++)
			{
				...
				if (claseAlgoritmo.getMetodo(j).getMarcadoProcesar() &&	esMetodoEscogido(claseAlgoritmo.getMetodo(j),metodos[i]))
				{
					// El tratamiento de los métodos varía sólo en la generación de
					// líneas de salida, por lo que en ambas opciones se generan de
					// de la misma manera las líneas de entrada (mediante el método
					...
					else if (claseAlgoritmo.getMetodo(j).getTecnica()==MetodoAlgoritmo.TECNICA_DYV)
					{
						if (tipoRetornoMetodo(metodos[i]).equals("void"))
							tratamientoMetodosVoidDYV(d,metodos[i],
									claseAlgoritmo.getMetodo(j).getIndiceEstructura()-1,
	...
	{
		if (depurar) System.out.println("____  esMetodoEscogido  ____");
	
		// Revisamos el nombre 
		String nombreM = m.getNombre();
		String nombreE = e.getAttribute("name");
		
		if (depurar) System.out.println(" - Nombre: ["+m.getNombre()+"]");
		if (depurar) System.out.println("   Nombre: ["+e.getAttribute("name")+"]");
		...
		for (int i=0; i<paramE.length; i++)
			clasesParamE[i]=ManipulacionElement.getChildElements(paramE[i])[0].getAttribute("name");
		
		if (depurar) System.out.println(" - NumParametros: ["+clasesParamM.length+"]");
		if (depurar) System.out.println("   NumParametros: ["+clasesParamE.length+"]");
		...
		{
			if (depurar) System.out.println("   TipoParam("+i+"): ["+clasesParamM[i]+"] vs ["+clasesParamE[i]+"]");
		...
		for (int i=0; i<m.getNumeroParametros(); i++)
		{
			...
			int dimE;
			try {
				dimE=Integer.parseInt(ManipulacionElement.getChildElements(paramE[i])[0].getAttribute("dimensions"));
			} catch (java.lang.NumberFormatException nfe) {
				dimE=0;
			}
			if (depurar) System.out.println("   DimParam("+i+"): ["+dimM+"] vs ["+dimE+"]");
			if (dimM!=dimE)
				return false;
	
		}
		// Habría que revisar el tipo de salida antes de salir  pero no hace falta:
		// no puede haber dos métodos con los mismos tipos de parámetro y distinto tipo de salida
		
		if (depurar) System.out.println("   ----> Son iguales ----<");
		return true;
	}
	
	
	/**
		Método auxiliar que gestiona la inserción de líneas en el código original Java
		
		@param d Document al que se añadirán nodos
		@param metodo Método al que se añaden nodos adicionales para las líenas de traza
	*/
	private static void tratamientoMetodosVoidREC(Document d, Element metodo)
	{
		...
		// Ahora nos ocupamos de las lineas de salida, basándonos en si hay o no nodos return
		if (returns.length==0)
		{
			// 01. "Object rrrrrr01[]=new Object[numParam];"
			Element sentenciaDeclObjectrrrrrr01= elementoDeclaracion(d, "Object","rrrrrr01",argumentos.length);
			
			// 02. "rrrrrr01[n]=parametroN;" (varias)
			Element sentenciaAsigParam[]=new Element[argumentos.length];
			for (int i=0; i<argumentos.length; i++)
				sentenciaAsigParam[i]=elementosAsigParametros(d, i,argumentos[i].getAttribute("name"), "rrrrrr01");
				
			// 03. Linea de Traza para la salida
			Element sentenciaAnadirSalida =Transformador.elementoAnadirSalida(d, "anadirSalida" ,"rrrrrr01",false);
			...
			Element sentenciasBloqueMetodo[]=ManipulacionElement.getChildElements(bloqueMetodo);
		...
		{
			// Habrá que insertar tantos conjuntos de lineas de salida para cada return que haya así como para el final del método
			
			if (depurar) System.out.println("MetodoVoidconReturn  01");
			// 01. "Object rrrrrr01[]=new Object[numParam];"
			Element sentenciaDeclObjectrrrrrr01= elementoDeclaracion(d, "Object","rrrrrr01",argumentos.length);
			
			if (depurar) System.out.println("MetodoVoidconReturn  02");
			// 02. "rrrrrr01[n]=parametroN;" (varias)
			Element sentenciaAsigParam[]=new Element[argumentos.length];
			for (int i=0; i<argumentos.length; i++)
				sentenciaAsigParam[i]=elementosAsigParametros(d, i,argumentos[i].getAttribute("name"), "rrrrrr01");
			
			if (depurar) System.out.println("MetodoVoidconReturn  03");
			// 03. Linea de Traza para la salida
			Element sentenciaAnadirSalida =Transformador.elementoAnadirSalida(d, "anadirSalida" ,"rrrrrr01",false);
			...
			{
				if (depurar) System.out.println("MetodoVoidconReturn  04");
				...
				if (depurar) System.out.println("MetodoVoidconReturn  05 padreReturn="+padreReturn.getNodeName());
				padreReturn.appendChild(sentenciaDeclObjectrrrrrr01.cloneNode(true));
				for (int j=0; j<argumentos.length; j++)
					padreReturn.appendChild(sentenciaAsigParam[j].cloneNode(true));
				padreReturn.appendChild(sentenciaAnadirSalida.cloneNode(true));
				
				if (depurar) System.out.println("MetodoVoidconReturn  06");
				padreReturn.removeChild(returns[i]);
				padreReturn.appendChild(returns[i]);
				if (depurar) System.out.println("MetodoVoidconReturn  07");
			}
			
			if (hayReturnFinal==-1)
			{
				if (depurar) System.out.println("MetodoVoidconReturn  08");
				// Añadimos al bloque del método las lineas de salida generadas
				bloqueMetodo.appendChild(sentenciaDeclObjectrrrrrr01);
				for (int i=0; i<argumentos.length; i++)
					bloqueMetodo.appendChild(sentenciaAsigParam[i]);
				bloqueMetodo.appendChild(sentenciaAnadirSalida);
				if (depurar) System.out.println("MetodoVoidconReturn  09");
			}
		}
	}
	...
	*/
	private static void tratamientoMetodosConValorREC(Document d, Element metodo) 
	{
		...
		// Ahora nos ocupamos de las lineas de salida, basándonos en los nodos return existentes
		for (int j=0; j<returns.length; j++)
		{
			...
			Element tipoRetorno = d.createElement("type");	// Nodo que expresará el tipo de la variable
			
			Element nodoTipoRetornoMetodo=nodoTipoRetornoMetodo(metodo);
			String stringTipoVariable=tipoMetodo;
			if (nodoTipoRetornoMetodo.getAttribute("dimensions").length()>0)	// si aparece atributo "dimensions"
			{
				int dimensiones = Integer.parseInt( nodoTipoRetornoMetodo.getAttribute("dimensions") );
				//for (int k=0; k<dimensiones; k++)
				//	stringTipoVariable=stringTipoVariable+"[]";			// añadimos "[]" por cada dimension
				stringTipoVariable=stringTipoVariable+ServiciosString.cadenaDimensiones( dimensiones );
			}				

			tipoRetorno.setAttribute("name",stringTipoVariable);
			sentenciaVariableRet.appendChild(tipoRetorno);
			Element hijosReturn[]=ManipulacionElement.getChildElements(returns[j]);
			sentenciaVariableRet.appendChild( ((Element)(hijosReturn[0].cloneNode(true))) );

			// 02. "Object rrrrrr01[]=new Object[1];"
			Element sentenciaDeclObjectrrrrrr01= elementoDeclaracion(d, "Object","rrrrrr01",1);

			// 03. "rrrrrr01[0]=zzzzzz01;"
			Element asigParametros = elementosAsigParametros(d, 0, "zzzzzz01", "rrrrrr01");

			// 04. Linea de Traza para la salida
			Element sentenciaAnadirSalida =Transformador.elementoAnadirSalida(d, "anadirSalida" ,"rrrrrr01",true);
			
			// 05. Cambiamos sentencia return para que devuelva siempre la variable zzzzzz01
			...
			Element primeraSentenciaPadreReturn=ManipulacionElement.getChildElements(padreReturn)[0];
			if(depurar) System.out.println("  Return Paso01");
			
			//
			Element bmh[]=ManipulacionElement.getChildElements(padreReturn);
			if (depurar) for (int i=0; i<bmh.length; i++)
			...
			// Recolocamos las lineas para que las líneas recién añadidas estén al principio del método
			while (padreReturn.getFirstChild()!=primeraSentenciaPadreReturn)
			{
				Node nodo_aux=padreReturn.getFirstChild();
				padreReturn.removeChild(nodo_aux);
				padreReturn.appendChild(nodo_aux);
				if (depurar) System.out.println("buscando primeraSentenciaPadreReturn");
			}
			if (padreReturn.getFirstChild().getNodeName().equals("return"))
			{
				...
				padreReturn.removeChild(nodo_aux);
				padreReturn.appendChild(nodo_aux);
				if (depurar) System.out.println("recolocando sentencia return");
			}
		}	
	}
	...
	/**
		...
		@param metodo Método al que se añaden nodos adicionales para las líenas de traza
	*/
	private static void tratamientoMetodosVoidDYV(Document d, Element metodo, int paramEstructura, int paramIndices[])
	{
		Object valores[]=recoleccionInformacion( metodo);
		//String tipoMetodo=(String)valores[0];// valores[0]=String tipoMetodo
		Element returns[]=(Element [])valores[1];
		...
		generacionLineasEntrada (d, bloqueMetodo, argumentos, paramEstructura, paramIndices);
		...
		if (returns.length==0)
		{
			...
			Element sentenciaEstructura= elementoDeclaracionAsignacion(d, paramEstructura, "eeeeee02",
						(Element)(((Element)(bloqueMetodo.getParentNode())).getElementsByTagName("formal-arguments").item(0)));	
			...
			// 03. Linea de Traza para la salida
			Element sentenciaAnadirSalida =Transformador.elementoAnadirEntradaSalida(d, "anadirSalida" ,"rrrrrr01", "eeeeee02",
												paramEstructura, metodo.getAttribute("name"));
		...
		else
		{
			...
			// 03. Linea de Traza para la salida
			Element sentenciaAnadirSalida =Transformador.elementoAnadirEntradaSalida(d, "anadirSalida" ,"rrrrrr01", "eeeeee02",
													paramEstructura, metodo.getAttribute("name"), false);
	...
	/**
		...
		@param metodo Método al que se añaden nodos adicionales para las líenas de traza
	*/
	private static void tratamientoMetodosConValorDYV(Document d, Element metodo, int paramEstructura, int paramIndices[])
	{
		...
		for (int j=0; j<returns.length; j++)
		{
			...
			// 04. Linea de Traza para la salida
			Element sentenciaAnadirSalida =Transformador.elementoAnadirEntradaSalida(d, "anadirSalida" ,"rrrrrr01", "eeeeee02",
					paramEstructura/*(-1)*/, metodo.getAttribute("name"), true);
	...
	private static Element elementoDeclaracionAsignacion(Document d, int numParamestructura, String nombreVar, Element formalArguments)
	{
		Element argumentos[]=ManipulacionElement.nodeListToElementArray( formalArguments.getElementsByTagName("formal-argument") );
		...
		inicializador.setAttribute("name",""+argumentos[numParamestructura].getAttribute("name"));
	...
	/**
		...
		@param d Document al que pertenecen todos los nodos
		@param r Element de la sentencia return
		@param pr Element del padre de la sentencia return
	*/
	...
	/**
		...
		@param metodo Método sobre el cual se recopilará información
	...
	*/	
	private static Object[] recoleccionInformacion(Element metodo)
	{
		...
		valores[1]= ManipulacionElement.nodeListToElementArray(metodo.getElementsByTagName("return"));

		//-- Nodos de argumentos que recibe el método
		valores[2]= ManipulacionElement.nodeListToElementArray(metodo.getElementsByTagName("formal-argument"));
		valores[2]=limpiezaArgumentos((Element [])valores[2]);	// Ahora sólo tenemos nodos de argumentos válidos
					
		//-- Nodo bloque del método
		valores[3]=null;
		Element hijosMetodo[]=   ManipulacionElement.nodeListToElementArray( metodo.getChildNodes() );
			for (int k=0; k<hijosMetodo.length; k++)
				if (hijosMetodo[k].getNodeName().equals("block"))
					valores[3]=hijosMetodo[k];
					
		if (depurar) System.out.println("METODO "+metodo.getAttribute("name")+" ["+((String)valores[0])+"]");
		if (depurar) System.out.println("Numero de returns: "+((Element [])(valores[1])).length+
										"\nNumero de argumentos: "+((Element [])(valores[2])).length);
		if (depurar) for (int z=0; z<((Element [])valores[2]).length; z++)
			System.out.println("Argumento["+(z+1)+"]="+((Element [])(valores[2]))[z].getAttribute("name"));
	...
	/**
		...
		@param d Documento al que pertenecen todos los nodos
		@param bloqueMetodo nodo que representea el bloque de instrucciones del método
		@param argumentos valores que se asignarán en las líneas de traza
		@return primera sentencia que colocamos
	*/	
	private static Element generacionLineasEntrada (Document d, Element bloqueMetodo, Element argumentos[])
	{
		// Creamos todas las lineas de entrada que necesitamos
		
		// 01. "Object pppppp01[]=new Object[numParametros];"
		Element sentenciaDeclObjectpppppp01= elementoDeclaracion(d, "Object","pppppp01",argumentos.length);
		
		// 01b. "String nnnnnn01[]={ nombreParam1, nombreParam2, nombreParam3, ... };"
		...
		Element sentenciaAsigParam[]=new Element[argumentos.length];
		for (int i=0; i<argumentos.length; i++)
			sentenciaAsigParam[i]=elementosAsigParametros(d, i,argumentos[i].getAttribute("name"), "pppppp01");

		// 03. Linea de Traza para la entrada
		Element sentenciaAnadirEntrada =Transformador.elementoAnadirEntrada(d, "anadirEntrada" ,"pppppp01",
														((Element)(bloqueMetodo.getParentNode())).getAttribute("name"));

		// Ahora insertamos las lineas de entrada	
		//Element hijosBloqueMetodo[]=   ManipulacionElement.nodeListToElementArray( bloqueMetodo.getChildNodes() );
		...
		while (bloqueMetodo.getFirstChild()!=sentenciaDeclObjectpppppp01)
		{
		 ...
		 // Devolvemos primera sentencia que colocamos para que después sepamos cuál debe ir en primer lugar
	...
	/**
		...
		@param bloqueMetodo nodo que representea el bloque de instrucciones del método
		@param argumentos valores que se asignarán en las líneas de traza
		@param paramEstructura numero de parametro que alberga la estructura sobre la que actua el algoritmo
		@param paramIndices numeros de parámetros que delimitan la actuación del algoritmo en la estructura
		@return primera sentencia que colocamos
	*/	
	private static Element generacionLineasEntrada (Document d, Element bloqueMetodo,
									Element argumentos[], int paramEstructura, int paramIndices[])
	{
		// Creamos todas las lineas de entrada que necesitamos
		...
		Element sentenciaEstructura= elementoDeclaracionAsignacion(d, paramEstructura, "eeeeee01",
						(Element)(((Element)(bloqueMetodo.getParentNode())).getElementsByTagName("formal-arguments").item(0)));	
		...
		// 03. Linea de Traza para la entrada
		Element sentenciaAnadirEntrada =Transformador.elementoAnadirEntradaSalida(d, "anadirEntrada" ,"pppppp01", "eeeeee01",
									paramEstructura, ((Element)(bloqueMetodo.getParentNode())).getAttribute("name"));
	...
	private static Element elementoNombresParam(Document d, Element formalArguments)
	{
		Element argumentos[]=ManipulacionElement.nodeListToElementArray( formalArguments.getElementsByTagName("formal-argument") );
		...
		// Añadimos a la inicialización los nombres de los distintos parámetros del método
		for (int i=0; i<argumentos.length; i++)
		{
	...
	private static Element elementoIndices(Document d, int numIndices[], Element formalArguments)
	{
		Element argumentos[]=ManipulacionElement.nodeListToElementArray( formalArguments.getElementsByTagName("formal-argument") );
		...
		if (numIndices!=null && numIndices.length>0)
		{
			...
			for (int i=0; i<numIndices.length; i++)
			{
				...
				inicializador.appendChild(cadena);
				cadena.setAttribute("name",argumentos[numIndices[i]].getAttribute("name"));
			}
		}
		else
		{
	...
	/**
		Corrige nombres, como el de la clase, los constructores, ... para asegurarse de que la clase nueva compilará
		
		@param d Documento al que pertenecen todos los nodos
		@param fichero nombre del fichero (el mismo que el de la clase)
	*/
	public static void correccionNombres(Document d, String fichero, String inicioNombre, String codigounico)
	{
		...
		String directorio=((Element)nodosFicheros.item(0)).getAttribute("name").replace(fichero,"");
		
		
		if (nodosFicheros.getLength()>0)
			((Element)nodosFicheros.item(0)).setAttribute("name",directorio+inicioNombre+fich2);
		...
		Element eMetodosClase[] = ManipulacionElement.nodeListToElementArray(metodosClase);
		...
		if (nodosClase!=null && (nodosClase.item(0) instanceof Element))
			...
			((Element)nodosClase.item(0)).getAttribute("name")+codigounico);
		else
			if (nodosInterfaz!=null && (nodosInterfaz.item(0) instanceof Element))
				((Element)nodosInterfaz.item(0)).setAttribute("name",inicioNombre+
				((Element)nodosInterfaz.item(0)).getAttribute("name")+codigounico);
		...
		for (int i=0; i<nodosFicheros.getLength(); i++)
			if (nodosFicheros.item(i) instanceof Element)
				((Element)nodosFicheros.item(i)).setAttribute("name",
				inicioNombre+((Element)nodosFicheros.item(i)).getAttribute("name")+codigounico);
				
		
	}

	/**
		Elimina todos los comentarios del fichero XML (los comentarios java del fichero original ni siquiera han sido leidos)
		
		@param nodo Node que representa la cima del árbol al cual se le va a eliminar recursivamente los comentarios
		@param tipo tipo de nodo que representa los comentarios
	*/
	public static void borrarComentarios(Node nodo, short tipo)
	{
	...
	/**
		Devuelve el número de nodos que tienen un determinado atributo
		
		@param nl lista de nodos que se va a estudiar
		@param atributo nombre del atributo que se busca
		@return número de nodos que tienen un determinado atributo
	*/
	public static int numNodosConAtributo(NodeList nl, String atributo)
	{
		int valor=0;
		for (int x=0; x<nl.getLength(); x++)
			if (nl.item(x) instanceof Element)
				if (  ((Element)(nl.item(x))).getAttribute(atributo) != null )
					valor++;
		return valor;
	}
	...
	/**
		Devuelve en formato String el tipo que devuelve el método cuyo elemento se pasa como parámetro
		
		@param e Element del método que se va a estudiar
	...
	*/
	...
	/**
		Devuelve el nodo del tipo que devuelve el método cuyo elemento se pasa como parámetro
		
		@param e Element del método que se va a estudiar
		@return nodo del tipo que devuelve el método cuyo elemento se pasa como parámetro
	*/
	...
	/**
		Dada una lista de nodos hijos "formal-argument" de un método, quitamos los que pertenecen a sentencias catch
		(es decir, lo que no son argumentos reales del método)
		
		@param el lista de argumentos que será filtrada para devover sólo aquellos verdaderamente válidos
	...
	*/
	...
	protected static ClaseAlgoritmo crearObjetoClaseAlgoritmo(Document d)
	{
		...
		eClase=ManipulacionElement.nodeListToElementArray(d.getElementsByTagName("class"));
		
		if (eClase.length==0)
		{
			eClase=ManipulacionElement.nodeListToElementArray(d.getElementsByTagName("java-class-file"));
			String pathCompleto=eClase[0].getAttribute("name");
			
			//String path=pathCompleto.substring(0,pathCompleto.lastIndexOf(File.pathSeparator)+1);
			String nombreClase=(pathCompleto.substring(pathCompleto.lastIndexOf(File.pathSeparator)+1));
		...
		String nombreClase=nombreAux.substring(0,nombreAux.indexOf("__codSRec__"));
		
		
		e=ManipulacionElement.nodeListToElementArray(d.getElementsByTagName("java-class-file"));
		String pathClase=e[0].getAttribute("name").replace(".java","");
		
		pathClase=pathClase.substring(0,pathClase.indexOf("__codSRec__"));
		
		String auxNombre=pathClase.substring(pathClase.lastIndexOf("\\")+1,pathClase.length());
		...
		e=ManipulacionElement.nodeListToElementArray(d.getElementsByTagName("method"));
		...
		for (int i=0; i<e.length; i++)
		{
			...
			try {
				dimRetornoMetodo=Integer.parseInt( hijos[0].getAttribute("dimensions") );
			} catch (java.lang.NumberFormatException nfe) {
				dimRetornoMetodo=0;
			}
			...
			for (int j=0; j<h.length; j++)
			{
				...
				tipoParametrosMetodo[j]=ManipulacionElement.getChildElements(h[j])[0].getAttribute("name");
				
				String dim=ManipulacionElement.getChildElements(h[j])[0].getAttribute("dimensions");
				try {
			...
			m=new MetodoAlgoritmo(nombreMetodo, tipoMetodo, dimRetornoMetodo, nombreParametrosMetodo, tipoParametrosMetodo, dimParametrosMetodo);
		...
		int numFields=0;
		Element []hijosClase=ManipulacionElement.nodeListToElementArray(eClase[0].getChildNodes());
		...
		//DEBUG
		/*for (int i=0; i<variablesClase.length; i++)
		{
			System.out.println("variablesClase["+i+"]: "+variablesClase[i].getNombre()+","+variablesClase[i].getTipo()+"("+variablesClase[i].getDimensiones()+")");
		}*/
		...
		for (int i=0; i<e.length; i++)
		{
			...
			{
				//System.out.println(" NODO: ["+padre.getNodeName()+"]");
			...
			MetodoAlgoritmo metodoActual=MetodoAlgoritmo.getMetodo(metodosClase,datosMetodo,dimParamMetodo);
			
			
			
			// ...parámetros y variables locales 
			Variable[] parametrosMetodo=Transformador.getParametrosMetodo(ManipulacionElement.nodeListToElementArray(e[i].getChildNodes())[1]);
					
			Element[] elementosMetodo=ManipulacionElement.nodeListToElementArray(e[i].getElementsByTagName("block"));
			Variable[] variablesMetodo=Transformador.getVariablesMetodo( ManipulacionElement.nodeListToElementArray(elementosMetodo[0].getElementsByTagName("local-variable") ) );
			...
			/*
			//DEBUG
			for (int x=0; x<parametrosMetodo.length; x++)
				System.out.println("parametrosMetodo["+x+"]: "+parametrosMetodo[x].getNombre()+","+parametrosMetodo[x].getTipo()+"("+parametrosMetodo[x].getDimensiones()+")");

			//DEBUG
			for (int x=0; x<variablesMetodo.length; x++)
				System.out.println("variablesMetodo["+x+"]: "+variablesMetodo[x].getNombre()+","+variablesMetodo[x].getTipo()+"("+variablesMetodo[x].getDimensiones()+")");
			*/		
			
			
			// ...qué llamadas a sí mismo o a otros métodos contiene
			Element[] elementosSend=ManipulacionElement.nodeListToElementArray(e[i].getElementsByTagName("send"));
	...
	private static boolean esMetodoApropiado(MetodoAlgoritmo m)
	{
		...
		for (int i=0; i<m.getNumeroParametros(); i++)
		{
			...
			if (!m.getTipoParametro(i).equals("byte") && !m.getTipoParametro(i).equals("short") && !m.getTipoParametro(i).equals("int") &&
				!m.getTipoParametro(i).equals("long") && !m.getTipoParametro(i).equals("float") && !m.getTipoParametro(i).equals("double") &&
				!m.getTipoParametro(i).equals("char") && !m.getTipoParametro(i).equals("String") && !m.getTipoParametro(i).equals("boolean"))
				return false;
			
			
			// Comprobamos que sean de dimensiones 0 (valor único), 1 (array) ó 2 (matriz)
	...
	{
		/*
  			<field name="miVariable" visibility="protected" final="true" static="true">
            	<type primitive="true" name="int"/>
            	<literal-number kind="int" value="5"/>
        	</field>
		 */
		...
		for (int i=0; i<elementosField.length; i++)
		{
			...
			tipo=ManipulacionElement.nodeListToElementArray(elementosField[i].getChildNodes())[0].getAttribute("name");
			String dimension=ManipulacionElement.nodeListToElementArray(elementosField[i].getChildNodes())[0].getAttribute("dimensions");
	...
	private static Variable[] getParametrosMetodo(Element elementoFormalArguments)
	{
		/*
  			<formal-arguments>
		...
		*/
		...
		Element []argumentos=ManipulacionElement.nodeListToElementArray(elementoFormalArguments.getChildNodes());
		...
		for (int i=0; i<argumentos.length; i++)
		{
			...
			tipo=ManipulacionElement.nodeListToElementArray(argumentos[i].getChildNodes())[0].getAttribute("name");
			String dimension=ManipulacionElement.nodeListToElementArray(argumentos[i].getChildNodes())[0].getAttribute("dimensions");
			dim=(dimension.length()>0 ? Integer.parseInt(dimension) : 0 );
			variables[i]=new Variable(nombre,tipo, dim);
		}
	...
	private static Variable[] getVariablesMetodo(Element[] elementosLocalVariable)
	{
		/*
  			<local-variable name="miVariable">
                    <type primitive="true" name="int"/>
                    <literal-number kind="int" value="5"/>
            </local-variable>
		*/
		...
		for (int i=0; i<elementosLocalVariable.length; i++)
		{
			...
			tipo=ManipulacionElement.nodeListToElementArray(elementosLocalVariable[i].getChildNodes())[0].getAttribute("name");
			String dimension=ManipulacionElement.nodeListToElementArray(elementosLocalVariable[i].getChildNodes())[0].getAttribute("dimensions");
			dim=(dimension.length()>0 ? Integer.parseInt(dimension) : 0 );
	...
	private static MetodoAlgoritmo datosMetodoLlamada(Element e, Variable[] v, ClaseAlgoritmo c)
	{
		/*
			<send message="fibonacci">
                <arguments>
                    <binary-expr op="-">
                        <var-ref name="n"/>
		...
		Element argumentos[]=ManipulacionElement.nodeListToElementArray(ManipulacionElement.nodeListToElementArray(e.getElementsByTagName("arguments"))[0].getChildNodes());
	...
	private static int dimDato(Element e, Variable[] v, ClaseAlgoritmo c)
	{
		...
		else if (tipoNodo.equals("send"))
		{
			...
			{
				if (	e.getAttribute("message").equals("indexOf") || e.getAttribute("message").equals("lastIndexOf") ||
						e.getAttribute("message").equals("codePointAt") || 
						e.getAttribute("message").equals("size") || e.getAttribute("message").equals("length"))
					return 0;
			}
		}
	...
	private static String tipoDato(Element e, Variable[] v, ClaseAlgoritmo c)
	{
		...
		{
			Element hijo=ManipulacionElement.nodeListToElementArray(e.getChildNodes())[0];
	...
	private static String tipoAdecuado(String t1, String t2,String op)
	{
		...
		if (t1.equals("byte"))
		{
			if (t2.equals("byte") || t2.equals("short") || t2.equals("int") || t2.equals("long") || t2.equals("float") || t2.equals("double"))
				return t2;
		...
		else if (t1.equals("short"))
		{
			if (t2.equals("byte"))
				return t1;
			else if (t2.equals("short") || t2.equals("int") || t2.equals("long") || t2.equals("float") || t2.equals("double"))
				return t2;
			else if (t2.equals("String"))
				return "String";
			else if (t2.equals("char"))
		...
		else if (t1.equals("int"))
		{
			if (t2.equals("byte") || t2.equals("short"))
				return t1;
			else if (t2.equals("int") || t2.equals("long") || t2.equals("float") || t2.equals("double"))
				return t2;
			else if (t2.equals("String"))
				return "String";
			else if (t2.equals("char"))
		...
		else if (t1.equals("long"))
		{
			if (t2.equals("byte") || t2.equals("short") || t2.equals("int"))
				return t1;
			else if (t2.equals("long") || t2.equals("float") || t2.equals("double"))
				return t2;
			else if (t2.equals("String"))
				return "String";
			else if (t2.equals("char"))
		...
		{
			if (t2.equals("byte") || t2.equals("short") || t2.equals("int") || t2.equals("long"))
				return t1;
		...
		{
			if (t2.equals("byte") || t2.equals("short") || t2.equals("int") || t2.equals("long") || t2.equals("float"))
				return t1;
	...
	{
		/*
			<method name="nivel4" visibility="private">
	            <type primitive="true" name="int" dimensions="2"/>
	            <formal-arguments>
	                <formal-argument name="s">
	                    <type name="String"  dimensions="2"/>
	                </formal-argument>
	            </formal-arguments>
	            <block>
	            	...
		...
		Element argumentos[]=ManipulacionElement.nodeListToElementArray( e.getElementsByTagName("formal-argument") );
		...
		for (int i=0; i<argumentos.length; i++)
			datos[i+1]=ManipulacionElement.nodeListToElementArray( argumentos[i].getChildNodes() )[0].getAttribute("name");

		// es necesario añadir el tipo de dato que devuelve el método al final
		Element tipo=ManipulacionElement.nodeListToElementArray( e.getChildNodes() )[0];
		datos[datos.length-1]=tipo.getAttribute("name");
		
		return datos;
	}
	
	private static int[] dimMetodo(Element e)	// recogemos SOLO dimensiones de los parámetros (no del tipo de vuelta)
	{
		/*
			<method name="nivel4" visibility="private">
	            <type primitive="true" name="int" dimensions="2"/>
		...
		for (int i=0; i<argumentos.length; i++)
		{
			String datoDim=ManipulacionElement.nodeListToElementArray( argumentos[i].getChildNodes() )[0].getAttribute("dimensions");
			dimensionesParam[i]=( datoDim.length()>0 ? Integer.parseInt(datoDim) : 0 );	
		}
