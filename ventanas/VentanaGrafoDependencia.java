...
public class VentanaGrafoDependencia extends JFrame implements
	...
	public VentanaGrafoDependencia(Ventana ventana, DatosMetodoBasicos metodo) {
	        ...
	        this.grafoDependencia = new GrafoDependencia(this.ventana.trazaCompleta, this.metodo);
	        ...
	        this.setMinimumSize(new Dimension(Conf.anchoPantalla * PORCENTAJE_PANTALLA_MINIMO / 100,
	        		Conf.altoPantalla * PORCENTAJE_PANTALLA_MINIMO / 100));
	...
	private void ajustarGrafoATamanioVentana(boolean ajustarTamanioVentana) {	
		...
		Dimension tamanioGrafo = this.grafoDependencia.getTamanioRepresentacion();
		
		/* Recorremos las vistas activas por si se ha desplazado algún nodo */
		Rectangle rectangulo = new Rectangle(0, 0, tamanioGrafo.width, tamanioGrafo.height);
		for (CellView cell : this.representacionGrafo.getGraphLayoutCache().getAllViews()) {
			Rectangle2D nodoBounds = cell.getBounds();
			rectangulo = rectangulo.union(new Rectangle((int)nodoBounds.getX(), (int)nodoBounds.getY(),
		...
		if (ajustarTamanioVentana) {
			...
			int anchura = Math.min(Conf.anchoPantalla * PORCENTAJE_PANTALLA_MAXIMO / 100, tamanioGrafo.width + WINDOW_WIDTH_MARGIN);
			anchura = Math.max(this.getMinimumSize().width, anchura);
			
			int altura = Math.min(Conf.altoPantalla * PORCENTAJE_PANTALLA_MAXIMO / 100, tamanioGrafo.height + WINDOW_HEIGHT_MARGIN);
			altura = Math.max(this.getMinimumSize().height, altura);
			
			tamanioVentana = new Dimension(anchura, altura);
			this.setSize(tamanioVentana);
			int[] ubicacion = Conf.ubicarCentro(tamanioVentana.width, tamanioVentana.height);
		...
		double ratioAnchura = (tamanioVentana.getWidth() - WINDOW_WIDTH_MARGIN) / tamanioGrafo.getWidth();
		double ratioAltura = (tamanioVentana.getHeight() - WINDOW_HEIGHT_MARGIN) / tamanioGrafo.getHeight();
	...
	public boolean tabular(String expresionParaFila, String expresionParaColumna) {
		...
		boolean error = this.grafoDependencia.tabular(expresionParaFila, expresionParaColumna);
		if (!error) {
	        this.representacionGrafo = this.grafoDependencia.obtenerRepresentacionGrafo();
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.botones[0]) {
			...
			double scale = Math.max(this.representacionGrafo.getScale() - SCALE_INCREMENTO, MIN_SCALE);
			this.representacionGrafo.setScale(scale);
		} else if (e.getSource() == this.botones[3]) {
			double scale = Math.min(this.representacionGrafo.getScale() + SCALE_INCREMENTO, MAX_SCALE);
			this.representacionGrafo.setScale(scale);
		}
	}
